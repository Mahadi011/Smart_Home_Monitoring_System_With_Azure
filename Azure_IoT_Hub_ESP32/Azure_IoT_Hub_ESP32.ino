...
int out_data=0,in_data=0, personCount=0,flag_status=0;
...
static void generateTelemetryPayload()
{
   ...
   telemetry_payload = "{ \"Flag\": " + String(flag_status) + ", \"People_Count\": " + String(personCount) + " }";
...
static void sendTelemetry()
{
  ...
  if (esp_mqtt_client_publish(
          mqtt_client,
          "devices/people_count/messages/events/$.ct=application%2Fjson%3Bcharset%3Dutf-8",
...
void loop()
{
  out_data=digitalRead(OUTSENSOR);
  activeOutsensor(out_data);
  
  in_data=digitalRead(INSENSOR);
  activeInsensor(in_data);
